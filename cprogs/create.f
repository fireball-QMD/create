...
! Needed for determing expansion of Legendre polynomials.
        real*8 ctheta (ntheta_max)
        real*8 ctheta_weights (ntheta_max)
...
! distances needed for three center integrals
        real*8 dbc                      ! maximal bond charge distance
        real*8 dna                      ! maximal neutral atom distance
 
        real*8 etotatom (nspec_max)
        ...
        character*45 time_message
        character*70 what1, what2, what3
 
        character*2 atom (nspec_max)
        character*25 napot (nspec_max, 0:nsh_max)
        character*25 ppfile (nspec_max)
        character*25 wavefxn (nspec_max, nsh_max)
        character*70 what (nspec_max)
...
! **********************************************************************
     ...
     1                  rcutoff(ispec,issh), xnocc(issh,ispec),
     2                  nzx(ispec), wavefxn(ispec,issh), iammaster)
         end do
        end do
...
! jel-PP
     ...
     1                 fraction, iammaster)
        end do
 
        if (iexc .eq. 4 .or. iexc .eq. 5 .or. iexc .eq. 6
     1      .or. iexc .eq. 10) then
         if (iammaster) then
...
! Read the neutral and non-neutral atom potentials
     ...
     1                 napot(ispec,0), etotatom(ispec), iammaster)
         do issh = 1, nssh(ispec)
          call readvnn (ispec, issh, rcutoff(ispec,issh),
     1                  nzx(ispec), napot(ispec,issh), etotatom(ispec),
     2                  iammaster)
         end do
        end do
...
! for each itype1, itype2 pair.
     ...
     1                   nssh, lssh, nleft, lleft, mleft, nright,
...
! Write out the results of mk_index
          if (iammaster) then
           ...
           do index = 1, index_max2c(itype1,itype2)
            write (*,501) index,
     1       nleft(itype1,itype2,index), lleft(itype1,itype2,index),
     2       mleft(itype1,itype2,index), nright(itype1,itype2,index),
     3       lright(itype1,itype2,index), mright(itype1,itype2,index)
           end do
...
! For the pseudopotential call a different mk_index routine
     ...
     1                     inter_max, nssh, nsshPP, lssh, lsshPP,
     2                     nleftPP, lleftPP, mleftPP, nrightPP,
     3                     lrightPP, mrightPP, index_max2cPP)
         end do
        end do
...
446       continue
     ...
     1                   (iswitch(interaction), interaction = 0, 11),
     2                   ibcna, ibcxc, inuxc1c, inuxc2c, isnuxc1c,
     3                   isnuxc2c    
         close (unit = 12)
         write (*,*) '  '
...
! jel-dq
     ...
     1                          iexc_new, fraction, nsshxc, 
     2                          rcutoffa_max, xnocc, dqorb, iderorb,    
     3                          what, signature, drr_rho)
          else 
           write (*,*) '  '
...
! thus use iexc_new instead of iexc.
     ...
     1                       iexc_new, interaction, nzx, nssh, n1, l1,
     2                       m1, n2, l2, m2, rcutoff1, rcutoff2,
     3                       rcutoff3, atom1, atom2, atom3, what1,
     4                       what2, what3, dbc, dna, signature,
     5                       ctheta, ctheta_weights, iderivtmp,
     6                       iderivtmp, 1, iammaster, ispher)
...
! Do not parallelize over ispnum, because it involves little work
     ...
     1                       interaction, nzx, nssh, n1, l1, m1,
     2                       n2, l2, m2, rcutoff1, rcutoff2, rcutoff3,
     3                       atom1, atom2, atom3, what1, what2, what3,
     4                       dbc, dna, signature, ctheta, 
     5                       ctheta_weights, isorpmin,
     6                       isorpmax, ispnum, iammaster, ispher)
...
! Do not parallelize over ispnum, because it involves little work
     ...
     3                       atom1, atom2, atom3, what1, what2, what3,
     4                       dbc, dna, signature,
     5                       ctheta, ctheta_weights, isorpmin,
     6                       isorpmax, ispnum, iammaster, ispher)
          end if ! end MPI which node
         end do
        end if
...
! **********************************************************************
     ...
     1                   nzx1, nzx2, rcutoff1, rcutoff2, nssh, lssh,
     2                   index_max, n1, l1, m1, n2, l2, m2, signature,
     3                   iammaster)
          end if
...
! **********************************************************************
     ...
     2                     nzx1, nzx2, rcutoff1, rcutoff2, nssh2, nzs,
     3                     nrhos, ndds, index_max, n1, l1, m1, n2, l2,
...
!     2                     nzx1, nzx2, rcutoff1, rcutoff2, nssh2PP,
     2                     nzx1, nzx2, rcutoff1, rcPP(itype2), nssh2PP,
     3                     nznl, nrhonl, nddnl, index_maxPP, n1PP, l1PP,
     4                     m1PP, n2PP, l2PP, m2PP, signature, iammaster,
     5                     ispher)
          end if
...
! Only do derivatives for idogs .eq. 1
           ...
           do ideriv = minderiv2c, maxderiv2c
            call twocenter (interaction, isorp, ideriv, iexc, fraction,
     1                      itype1, itype2, atom1, atom2, what1, what2,
     2                      nzx1, nzx2, rcutoff1, rcutoff2, nssh2,
     3                      nzxco, nrhoxco, nddxco, index_maxsp, n1sp,
     4                      l1sp, m1sp, n2sp, l2sp, m2sp, signature,
     5                      iammaster, ispher)
           end do
          end if
...
! we will use Ceperly/Alder (iexc = 3) for the nu part.
     ...
     2                     what1, what2, nzx1, nzx2, rcutoff1, rcutoff2,
     3                     nssh2, nzeh, nrhoeh, nddeh, index_max, n1,
     4                     l1, m1, n2, l2, m2, signature, iammaster,
     5                     ispher)
          end if
...
! **********************************************************************
          if (isnuxc2c .eq. 1) then
           if (iexc .eq. 11) then 
            ...
            call twocenter (interaction, isorp, ideriv, iexc, fraction,
     1                      itype1, itype2, atom1, atom2, what1, what2, 
     2                      nzx1, nzx2, rcutoff1, rcutoff2, nssh2, nzeh,
     3                      nrhoeh, nddeh, index_max, n1, l1, m1, n2, 
     4                      l2, m2, signature, iammaster, ispher) 
           else 
...
! We do not calculate charge interactions if idogs .ne. 1
     ...
     1                       itype1, itype2, atom1, atom2, what1, what2,
     2                       nzx1, nzx2, rcutoff1, rcutoff2, nssh2,
     3                       nznao, nrhonao, nddnao, index_max, n1,
     4                       l1, m1, n2, l2, m2, signature, iammaster,
     5                       ispher)
            end do
           end do
...
! Only do derivatives for idogs .eq. 1
     ...
     1                      itype1, itype2, atom1, atom2, what1, what2,
     2                      nzx1, nzx2, rcutoff1, rcutoff2, nssh2,
     3                      nzxco, nrhoxco, nddxco, index_max, n1,
     4                      l1, m1, n2, l2, m2, signature, iammaster,
     5                      ispher)
           end do
...
! a separate file.
     ...
     1                         nssh, itype1, itype2, atom1, atom2,
     2                         what1, what2, nzx1, nzx2, rcutoff1,
     3                         rcutoff2, nzexo, nrhoexo, nddexo,
     4                         index_max, inter_max, nalpha, lalpha,
     5                         malpha, n1, l1, m1, n2, l2, m2,
     6                         signature, iammaster)
...
! We do not calculate charge interactions if idogs .ne. 1
     ...
     1                      itype1, itype2, atom1, atom2, what1, what2,
     2                      nzx1, nzx2, rcutoff1, rcutoff2, nssh2,
     3                      nznat, nrhonat, nddnat, index_max, n1,
     4                      l1, m1, n2, l2, m2, signature, iammaster,
     5                      ispher)
           end do
          end if
...
! Only do derivatives for idogs .eq. 1
     ...
     1                      itype1, itype2, atom1, atom2, what1, what2,
     2                      nzx1, nzx2, rcutoff1, rcutoff2, nssh2,
     3                      nzxca, nrhoxca, nddxca, index_max, n1,
     4                      l1, m1, n2, l2, m2, signature, iammaster,
     5                      ispher)
           end do
...
! a separate file.
     ...
     1                       itype1, itype2, atom1, atom2, what1,
     2                       what2, nzx1, nzx2, rcutoff1, rcutoff2,
     3                       nzexa, nrhoexa, nddexa, index_max,
     4                       inter_max, nalpha, lalpha, malpha, n1, l1,
     5                       m1, n2, l2, m2, signature, iammaster)
             end do
            end do
           end if
